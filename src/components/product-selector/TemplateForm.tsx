            ...
            return (
              <div key={category} className="border border-gray-700 rounded-lg overflow-hidden bg-gray-800/50">
                <button
                  ...
                  className="w-full px-3 py-2.5 bg-gray-800 hover:bg-gray-750 transition-colors flex items-center justify-between"
                >
                  <div className="flex items-center gap-2">
                    <Icon className="w-4 h-4 text-purple-400" />
                    <div className="text-left">
                      <h4 className="font-semibold text-gray-100 text-xs">{category}</h4>
                      <p className="text-[10px] text-gray-400 mt-0.5 line-clamp-1">{categoryDescriptions[category]}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-1.5">
                    <span className="text-[10px] text-gray-400 font-medium">{categoryTemplates.length}</span>
                    {isExpanded ? (
                    ...
                    {categoryTemplates.map((template, index) => {
                      ...
                      return (
                        <button
                          ...
                          disabled={isLocked}
                          className={`p-2.5 border-2 rounded-lg text-left transition-all relative ${
                            isLocked
                        ...
                        >
                          ...
                          <div className="flex items-start justify-between mb-1">
                            <h5 className="font-semibold text-gray-100 text-xs pr-2 flex items-center gap-1">
                              {isCustom && <Star className="w-3 h-3 text-amber-400 fill-amber-400" />}
                              {template.template_name}
                            </h5>
                            <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-semibold whitespace-nowrap ml-1 flex items-center gap-0.5 ${
                          ...
                          <p className="text-[10px] text-gray-400 leading-relaxed mb-1.5 line-clamp-2">
                            {getCleanDescription(template)}
                          </p>
                          <div className="flex gap-1 flex-wrap">
                            {template.keywords.slice(0, 2).map((keyword, ki) => (
                              <span key={ki} className="text-[10px] px-1 py-0.5 bg-gray-700 text-gray-300 rounded">
                                {keyword}
                              </span>
                            ))}
                          </div>
