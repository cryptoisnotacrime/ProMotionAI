  ...
  const getBrandColors = (): string => {
    if (store.brand_colors && store.brand_colors.length > 0) {
      const colorNames = store.brand_colors
        ...
        .filter(Boolean)
        .filter((name: string) => !/^#[0-9A-Fa-f]{3,6}$/.test(name)); // Filter out any hex codes

      // Deduplicate color names (case-insensitive)
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        ...
        <div>
          <h3 className="text-sm font-semibold text-gray-100 mb-2">Select Template</h3>
          <p className="text-xs text-gray-400 mb-3">Choose a style that best fits your product and brand</p>
          ...
          {Object.entries(groupedTemplates).map(([category, categoryTemplates]) => {
            ...
            return (
              <div key={category} className="border border-gray-700 rounded-lg overflow-hidden bg-gray-800/50">
                <button
                  onClick={() => toggleCategory(category)}
                  className="w-full px-3 py-3 bg-gray-800 hover:bg-gray-750 transition-colors flex items-center justify-between min-h-[60px]"
                >
                  <div className="flex items-center gap-2.5 flex-1">
                    <Icon className="w-4 h-4 text-purple-400 flex-shrink-0" />
                    <div className="text-left flex-1 min-w-0">
                      <h4 className="font-semibold text-gray-100 text-xs mb-0.5">{category}</h4>
                      <p className="text-[10px] text-gray-400 line-clamp-1">{categoryDescriptions[category]}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 flex-shrink-0 ml-2">
                    <span className="text-[10px] text-gray-400 font-medium bg-gray-700/50 px-1.5 py-0.5 rounded">{categoryTemplates.length}</span>
                    {isExpanded ? (
                    ...
                    {categoryTemplates.map((template, index) => {
                      ...
                      return (
                        <button
                          ...
                          disabled={isLocked}
                          className={`w-full min-h-[90px] p-3 border-2 rounded-lg text-left transition-all relative flex flex-col ${
                            isLocked
                        ...
                        >
                          ...
                          <div className="flex items-start justify-between mb-1.5">
                            <h5 className="font-semibold text-gray-100 text-xs pr-2 flex items-center gap-1 line-clamp-1">
                              {isCustom && <Star className="w-3 h-3 text-amber-400 fill-amber-400 flex-shrink-0" />}
                              <span className="line-clamp-1">{template.template_name}</span>
                            </h5>
                            <span className={`flex-shrink-0 text-[10px] px-1.5 py-0.5 rounded-full font-semibold whitespace-nowrap ml-1 flex items-center gap-0.5 ${
                              ...
                              {!isCustom && (isPro || isBasic) && <Crown className="w-2.5 h-2.5" />}
                              {isCustom ? 'CUSTOM' : template.meta.tier.toUpperCase()}
                            </span>
                          </div>
                          <p className="text-[10px] text-gray-400 leading-relaxed mb-2 line-clamp-2 flex-grow">
        ...
        {selectedTemplate && (
          <div>
            <h3 className="text-sm font-semibold text-gray-100 mb-2">Customize Settings</h3>
            <p className="text-xs text-gray-400 mb-3">Fine-tune the video generation parameters</p>
            ...
            <div className="space-y-3">
              <div>
                ...
                <select
                  ...
                  className="w-full px-2 py-1.5 text-xs border border-gray-700 rounded focus:ring-1 focus:ring-purple-500 bg-gray-800 text-gray-100"
                >
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-2 py-1.5 text-xs border border-gray-700 rounded focus:ring-1 focus:ring-purple-500 bg-gray-800 text-gray-100 placeholder-gray-500"
                />
              </div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  >
                    ...
                    <div className="text-center">
                      <span className="text-[10px] font-semibold block">9:16 Vertical</span>
                      <span className="text-[9px] text-gray-400">IG, TikTok</span>
                    </div>
                  </button>
                  <button
                    type="button"
                  ...
                  >
                    ...
                    <div className="text-center">
                      <span className="text-[10px] font-semibold block">16:9 Horizontal</span>
                      <span className="text-[9px] text-gray-400">YouTube, Web</span>
                    </div>
                  </button>
                </div>
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={2}
                  className="w-full px-2 py-1.5 text-xs border border-gray-700 rounded focus:ring-1 focus:ring-purple-500 resize-y bg-gray-800 text-gray-100 placeholder-gray-500"
                />
              </div>
            </div>
          </div>
